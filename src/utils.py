...
def get_data_loader(name='cloud', train=True, batch_size=64, split='train', inp_size=224, analysis=False):
    if name == 'cloud':
        from cloud_dataset import CloudDataset
        dataset = CloudDataset(split, inp_size, analysis=analysis)
    else:
...
def eval_dataset_map(model, device, test_loader):
    ...
    with torch.no_grad():
        ...
        for data, target, wgt in test_loader:
            ...
            gt, pred, valid = target.cpu().detach().numpy(), p.cpu().detach().numpy(), wgt.cpu().detach().numpy() 
            p_list.append(pred)
            v_list.append(valid)
            gt_list.append(gt)

    pred = np.concatenate(p_list, axis=0)
